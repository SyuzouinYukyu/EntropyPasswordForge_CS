...
- バージョン: **v1.0.11**
- 実行ファイル名: `EntropyPasswordForge_CS_v1.0.11.exe`
...
- 30秒後のクリップボード自動消去。
- 60秒後の生成結果表示自動消去。
- 生成結果欄のみを対象にしたフォントサイズ変更。
- 生成結果画面直上の境界ドラッグによる、上部設定エリアと生成結果エリアの高さ調整。
- `記号・配分` タブ内のスライダーが切れにくい AutoScroll 対応レイアウト。
- 前回使用したUI設定の復元。
...
生成結果画面直上の境界をドラッグすると、上部設定エリアと生成結果エリアの高さを調整できます。生成結果エリアとログエリアの境界も従来通りドラッグで調整できます。固定境界はドラッグ可能に見えないようにしています。

`記号・配分` タブの文字配分スライダーは、親コンテナ、行高、最小サイズ、AutoScroll を含めて調整しています。ウィンドウサイズや分割境界の変更で高さが不足する場合は、タブ内スクロールで確認できます。

## 設定保存

前回使用したUI設定は、次回起動時に復元されます。設定ファイルはユーザー別のアプリ設定フォルダに JSON 形式で保存します。

```text
%APPDATA%\EntropyPasswordForge_CS\settings.json
```

保存対象は、パスワード長、生成個数、文字種チェック、記号設定、最低使用数、結果フォントサイズ、分割境界位置、ウィンドウサイズと位置、選択中タブなどのUI設定のみです。

設定ファイルには、生成済みパスワード、生成結果欄の本文、ログ本文、クリップボード内容、マウスエントロピー生データ、CPUジッター生データ、seed、HMAC key、DRBG内部状態などの秘密情報は保存しません。
...
```powershell
dotnet restore .\EntropyPasswordForge_CS_v1.0.11.csproj
dotnet build .\EntropyPasswordForge_CS_v1.0.11.csproj -c Release
dotnet publish .\EntropyPasswordForge_CS_v1.0.11.csproj -c Release -r win-x64 --self-contained true /p:PublishSingleFile=true
```

発行後の実行ファイルは、通常次の場所に出力されます。

```text
bin\Release\net8.0-windows\win-x64\publish\EntropyPasswordForge_CS_v1.0.11.exe
```

## リリースハッシュ

v1.0.11 公開用実行ファイルの SHA-256 は次の通りです。

```text
DB927A35D11FCEDC45E12801F09B54F06D0715CB5659007A4D2BAA7D8407DFEE
```
